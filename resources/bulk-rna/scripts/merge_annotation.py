...
def add_spikeins_to_file(spikeins_fn, f):
    with gzip.open(spikeins_fn, "rt") as spikeins_f:
        spikein_string = spikeins_f.read()
...
if __name__ == "__main__":
    ...
    with gzip.open(args.output_filename, "wt") as f_out:
        ...
        if args.tRNA is not None:
            add_tRNA_to_file(args.tRNA, f_out)
        if args.spikeins is not None:
            add_spikeins_to_file(args.spikeins, f_out)
